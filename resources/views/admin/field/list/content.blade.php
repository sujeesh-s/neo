                        ...
                        <div id="table_body" class="card-body table-card-body">
                            <div>
                                <table id="attribute" class="attribute-table table table-striped table-bordered w-100 text-nowrap">
                                    ...
                                    <tbody>
                                        @if($fields && count($fields) > 0) @php $n = 0; @endphp
                                            @foreach($fields as $row) @php $n++; @endphp
                                                @php 
                                                ...
                                                <tr class="dtrow" id="dtrow-{{$row->id}}">
                                                    ...
                                                    <td class="text-center">
                                                        @if(checkPermission('admin/fields','edit') == true)
                                                        <button id="editForm-{{$row->id}}" class="mr-2 btn btn-info btn-sm editForm"><i class="fa fa-edit mr-1"></i>Edit</button>
                                                        @endif
                                                        @if(checkPermission('admin/fields','delete') == true)
                                                        <button id="delBtn-{{$row->id}}" class="mr-2 btn btn-secondary btn-sm delBtn" <?php if (stripos($row->name, 'carat') !== false || stripos($row->name, 'Karat') !== false){?> disabled <?php }?>><i class="fe fe-trash-2 mr-1"></i>Delete</button>
                                                        @endif
                                                    </td>
                                            @endforeach
                                        @endif
